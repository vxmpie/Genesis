...
function ResetModule.StartTracker(State, countdownLabel, statusLabel)
    ...
    trackerThread = task.spawn(function()
        local lastPos = nil
        local idleSeconds = 0
        local timerCountdown = State.IntervalSeconds or 15
            ...
            if not hrp or not humanoid or humanoid.Health <= 0 then
                ...
                if State.Mode == "Anti-Stuck" then
                    ...
                    if distance > 3 then
                        ...
                        local limit = State.IntervalSeconds or 15
                        local remaining = math.max(0, limit - idleSeconds)
                        ...
                        if remaining <= 0 then
                            ...
                            if statusLabel then
                                statusLabel.Text = string.format("IDLE (%ds / %ds)", idleSeconds, limit)
                    ...
                    if timerCountdown <= 0 then
                        ...
                        ResetModule.ResetCharacter()
                        task.wait(3)
                        timerCountdown = State.IntervalSeconds or 15
                        if statusLabel then
